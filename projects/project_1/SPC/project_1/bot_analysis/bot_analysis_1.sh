...
psql -d fdd2db -c 'select cast(count(distinct sbq_2.user_id) as float)/cast(count(distinct interval_averages_by_genre.user_id) as float) as percentage_of_repeated_ids from (select distinct user_id, count(user_id) over(partition by user_id) as number_of_appearances, genre from possible_bots) as sbq_2 full join interval_averages_by_genre on sbq_2.genre = interval_averages_by_genre.genre where number_of_appearances>=2' | awk -F '\|' '/^ +[0-9]+/{print $1 "," $2}'| sed -E "s/ //g" > percentage_1.csv
